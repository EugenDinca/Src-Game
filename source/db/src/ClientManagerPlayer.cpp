...
bool CreateItemTableFromRes(MYSQL_RES* res, std::vector<TPlayerItem>* pVec, DWORD dwPID)
{
	...
	for (int i = 0; i < rows; ++i)
	{
		...
		for (int z = 0; z < ITEM_SOCKET_MAX_NUM; z++)
		{
			str_to_number(item.alSockets[z], row[cur++]);
		}
...
size_t CreatePlayerSaveQuery(char* pszQuery, size_t querySize, TPlayerTable* pkTab)
{
	...
	CDBManager::instance().EscapeString(text, &pkTab->biolog, sizeof(pkTab->biolog));
	queryLen += snprintf(pszQuery + queryLen, querySize - queryLen, ",biolog = '%s' ", text);
#endif
#ifdef ENABLE_CHAR_SETTINGS
	CDBManager::instance().EscapeString(text, &pkTab->char_settings, sizeof(pkTab->char_settings));
	queryLen += snprintf(pszQuery + queryLen, querySize - queryLen, ",char_settings = '%s' ", text);
#endif
#ifdef ENABLE_DUNGEON_TURN_BACK
	CDBManager::instance().EscapeString(text, &pkTab->dungeonInfo, sizeof(pkTab->dungeonInfo));
	queryLen += snprintf(pszQuery + queryLen, querySize - queryLen, ",dungeon_info = '%s' ", text);
#endif
#ifdef ENABLE_PLAYER_STATS_SYSTEM
	CDBManager::instance().EscapeString(text, &pkTab->player_stats, sizeof(pkTab->player_stats));
	queryLen += snprintf(pszQuery + queryLen, querySize - queryLen, ",player_stats = '%s' ", text);
#endif
...
#endif
				"FROM item%s WHERE owner_id=%d AND (`window` in ('INVENTORY','EQUIPMENT','DRAGON_SOUL_INVENTORY',"
...
#ifdef ENABLE_BIOLOG_SYSTEM
	if (row[col])
		thecore_memcpy(&pkTab->biolog, row[col], sizeof(pkTab->biolog));
	else
		memset(&pkTab->biolog, 0, sizeof(pkTab->biolog));
	col++;
#endif

#ifdef ENABLE_CHAR_SETTINGS
	if (row[col])
		thecore_memcpy(&pkTab->char_settings, row[col], sizeof(pkTab->char_settings));
	else
		memset(&pkTab->char_settings, 0, sizeof(pkTab->char_settings));
	col++;
#endif

#ifdef ENABLE_DUNGEON_TURN_BACK
	if (row[col])
		thecore_memcpy(&pkTab->dungeonInfo, row[col], sizeof(pkTab->dungeonInfo));
	else
		memset(&pkTab->dungeonInfo, 0, sizeof(pkTab->dungeonInfo));
	col++;
#endif

#ifdef ENABLE_PLAYER_STATS_SYSTEM
	if (row[col])
		thecore_memcpy(&pkTab->player_stats, row[col], sizeof(pkTab->player_stats));
	else
		memset(&pkTab->player_stats, 0, sizeof(pkTab->player_stats));
	col++;
#endif
...
#endif
#ifdef ENABLE_CHAT_COLOR_SYSTEM
	str_to_number(pkTab->chatColor, row[col++]);
#endif
...
void CClientManager::__QUERY_PLAYER_CREATE(CPeer* peer, DWORD dwHandle, TPlayerCreatePacket* packet)
{
...
#ifdef ENABLE_BIOLOG_SYSTEM
	CDBManager::instance().EscapeString(text, &packet->player_table.biolog, sizeof(packet->player_table.biolog));
	queryLen += snprintf(queryStr + queryLen, sizeof(queryStr) - queryLen, ", '%s'", text);
#endif
#ifdef ENABLE_CHAR_SETTINGS
	CDBManager::instance().EscapeString(text, &packet->player_table.char_settings, sizeof(packet->player_table.char_settings));
	queryLen += snprintf(queryStr + queryLen, sizeof(queryStr) - queryLen, ", '%s'", text);
#endif
#ifdef ENABLE_DUNGEON_TURN_BACK
	CDBManager::instance().EscapeString(text, &packet->player_table.dungeonInfo, sizeof(packet->player_table.dungeonInfo));
	queryLen += snprintf(queryStr + queryLen, sizeof(queryStr) - queryLen, ", '%s'", text);
#endif
#ifdef ENABLE_PLAYER_STATS_SYSTEM
	CDBManager::instance().EscapeString(text, &packet->player_table.player_stats, sizeof(packet->player_table.player_stats));
	queryLen += snprintf(queryStr + queryLen, sizeof(queryStr) - queryLen, ", '%s'", text);
#endif
	...
	snprintf(queryStr, sizeof(queryStr), "INSERT INTO log.player_hack (pid, name, hack_count, bot_count) VALUES(%u, '%s', 0, 0)",
		player_id, packet->player_table.name);
