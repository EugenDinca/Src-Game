...
typedef struct SValueNames
{
	const char* c_pszName;
	long		lValue;
} TValueNames;

TValueNames aApplyTypeNames[] =
{
	{ "STR",		APPLY_STR		},
	{ "DEX",		APPLY_DEX		},
	{ "CON",		APPLY_CON		},
...
long get_apply_type(const char* apply_type_string)
{
	TValueNames* value_name;
	for (value_name = aApplyTypeNames; value_name->c_pszName; ++value_name)
	{
...
bool CClientManager::InitializeTables()
{
	...
	if (!InitializeRank())
	{
		sys_err("Initialize Ranking FAILED");
	}
#endif
#ifdef ENABLE_IN_GAME_LOG_SYSTEM
	if (!InitializeInGameLogTable())
	{
		sys_err("Initialize InGameLogTable FAILED");
		return false;
	}
#endif
...
bool CClientManager::InitializeMobTable()
{
	...
	while (data.Next())
	{
		...
		sys_log(1, "MOB #%-5d %-64s %-64s level: %-3u rank: %u empire: %d", mob_table->dwVnum, mob_table->szName, mob_table->szLocaleName, mob_table->bLevel, mob_table->bRank, mob_table->bEmpire);
		++mob_table; //@Lightwork131 24s -> 64s mob name lenght
	}
...
bool CClientManager::InitializeShopTable()
{
	...
	static const char* s_szQuery =
		...
		"ON shop.vnum = shop_item.shop_vnum ORDER BY shop.vnum, shop_item.itemOrder";
...
bool CClientManager::InitializeItemTable()
{
	...
	while (it != m_vec_itemTable.end())
	{
		...
		sys_log(1, "ITEM: #%-5lu %-64s %-64s VAL: %ld %ld %ld %ld %ld %ld WEAR %lu ANTI %lu IMMUNE %lu REFINE %lu REFINE_SET %u MAGIC_PCT %u",
			item_table->dwVnum,
			item_table->szName, //@Lightwork36 24s -> 64s item name lenght
...
#endif
#endif
		t.dwApplyIndex = get_apply_type(t.szApply);
...
bool CClientManager::InitializeItemRareTable()
{
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		str_to_number(t.lValues[4], data[col++]);
#if defined (ENABLE_PET_SKIN_ATTR)
		str_to_number(t.bMaxLevelBySet[ATTRIBUTE_SET_RARE_SKIN], data[col++]);
#endif
#if defined (ENABLE_HERSEYE_ATTIR)
		str_to_number(t.bMaxLevelBySet[ATTRIBUTE_SET_RARE_GLOVE], data[col++]);
		str_to_number(t.bMaxLevelBySet[ATTRIBUTE_SET_RARE_PENDANT], data[col++]);
		str_to_number(t.bMaxLevelBySet[ATTRIBUTE_SET_RARE_CROWN], data[col++]);
		str_to_number(t.bMaxLevelBySet[ATTRIBUTE_SET_RARE_BELT], data[col++]);

		str_to_number(t.bMaxLevelBySet[ATTRIBUTE_SET_RARE_COSTUME], data[col++]);

		str_to_number(t.bMaxLevelBySet[ATTRIBUTE_SET_RARE_DREAMSOUL], data[col++]);
		str_to_number(t.bMaxLevelBySet[ATTRIBUTE_SET_RARE_BOOSTERS], data[col++]);
		str_to_number(t.bMaxLevelBySet[ATTRIBUTE_SET_RARE_RINGS], data[col++]);
		str_to_number(t.bMaxLevelBySet[ATTRIBUTE_SET_RARE_SHININGS], data[col++]);

		str_to_number(t.bMaxLevelBySet[ATTRIBUTE_SET_RARE_PET_AND_MOUNT], data[col++]);
#endif
		t.dwApplyIndex = get_apply_type(t.szApply);
		sys_log(1,"rare idx %d apply %s value %d %d %d %d %d glove %d pend %d ", t.dwApplyIndex, t.szApply,t.lValues[0], t.lValues[1], t.lValues[2], t.lValues[3], t.lValues[4], t.bMaxLevelBySet[ATTRIBUTE_SET_RARE_GLOVE], t.bMaxLevelBySet[ATTRIBUTE_SET_RARE_PENDANT]);
	...
	for (itertype(m_vec_itemTable) it = m_vec_itemTable.begin(); it != m_vec_itemTable.end(); it++)
	{
		if (g_stLocaleNameColumn != "name")
		{
			...
			snprintf(query, sizeof(query),
				...
				t.aApplies[4].bType, t.aApplies[4].lValue,
#endif
...
bool CClientManager::InitializeMobTableFromDB()
{
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		sys_log(0, "MOB #%-5d %-64s %-64s level: %-3u rank: %u empire: %d", //@Lightwork131 24s -> 64s mob name lenght
			mob_table->dwVnum,
			mob_table->szName,
...
#ifdef ENABLE_GLOBAL_RANKING
static bool SortRanking(const TRanking& a, const TRanking& b)
{
	return (a.value > b.value);
}

bool CClientManager::InitializeRank()
{
	char szFetchQuery[128];
	memset(&szFetchQuery, 0, sizeof(szFetchQuery));
	snprintf(szFetchQuery, sizeof(szFetchQuery), "SELECT id, player_stats, alignment, playtime FROM player.player");

	std::unique_ptr<SQLMsg> pMsg(CDBManager::instance().DirectQuery(szFetchQuery));

	if (pMsg->Get()->uiNumRows == 0)
	{
		sys_err("Global ranking select query fail");
		return false;
	}

	//GM LÝST
	if (m_gmListPid.empty())
	{
		snprintf(szFetchQuery, sizeof(szFetchQuery), "SELECT mName FROM common.gmlist");
		std::unique_ptr<SQLMsg> pMsg2(CDBManager::instance().DirectQuery(szFetchQuery));

		if (pMsg2->Get()->uiNumRows != 0)
		{
			MYSQL_ROW row;
			while (nullptr != (row = mysql_fetch_row(pMsg2->Get()->pSQLResult)))
			{
				snprintf(szFetchQuery, sizeof(szFetchQuery), "SELECT id FROM player.player WHERE name = '%s' ", row[0]);
				std::unique_ptr<SQLMsg> pMsg3(CDBManager::instance().DirectQuery(szFetchQuery));
				if (pMsg3->Get()->uiNumRows != 0)
				{
					MYSQL_ROW row2 = mysql_fetch_row(pMsg3->Get()->pSQLResult);
					if (row2 != nullptr)
					{
						DWORD pid = 0;
						str_to_number(pid, row2[0]);
						m_gmListPid.insert(pid);
					}
				}
			}
		}
	...
	MYSQL_ROW row;
	std::vector<TRanking> rankList[StatsTypes::RANKING_MAX];

	while (nullptr != (row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
	{
		TPlayerStats tmpStats{};
		TRanking tmpRank{};

		str_to_number(tmpRank.pid, row[0]);

		if (m_gmListPid.find(tmpRank.pid) != m_gmListPid.end())
			continue;

		memcpy(&tmpStats, row[1], sizeof(tmpStats));

		for (int i = 0; i < StatsTypes::STATS_MAX; i++)
		{
			tmpRank.value = tmpStats.stat[i];
			rankList[i].push_back(tmpRank);
		}

		int alignment = 0;
		str_to_number(alignment, row[2]);
		if (alignment >= 10)
		{
			alignment /= 10;
			tmpRank.value = static_cast<DWORD>(alignment);
			rankList[StatsTypes::RANKING_ALIGN].push_back(tmpRank);
		}

		str_to_number(tmpRank.value, row[3]);
		if (tmpRank.value != 0)
			rankList[StatsTypes::RANKING_PLAY_TIME].push_back(tmpRank);
	}

	for (int i = 0; i < StatsTypes::RANKING_MAX; i++)
	{
		std::sort(rankList[i].begin(), rankList[i].end(), SortRanking);

		m_rankList[i].clear();
		m_rankList[i] = rankList[i];
	}

	TRankingData rankingData{};
	memset(&rankingData, 0, sizeof(TRankingData));

	for (int i = 0; i < StatsTypes::RANKING_MAX; i++)
	{
		int maxRank = m_rankList[i].size();
		maxRank = maxRank > RankingConf::RANKING_MAX_PLAYER_COUNT ? RankingConf::RANKING_MAX_PLAYER_COUNT : maxRank;
		for (int y = 0; y < maxRank; y++)
		{
			rankingData.rank[i][y].bOrder = y;
			rankingData.rank[i][y].llScore = m_rankList[i][y].value;

			snprintf(szFetchQuery, sizeof(szFetchQuery), "SELECT account_id, name, job FROM player.player WHERE id = %d", m_rankList[i][y].pid);

			std::unique_ptr<SQLMsg> pMsg(CDBManager::instance().DirectQuery(szFetchQuery));
			if (pMsg->Get()->uiNumRows == 0)
			{
				sys_err("Ranking select player info error player id %d ", m_rankList[i][y].pid);
				continue;
			}
			if (nullptr == (row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
			{
				sys_err("Ranking select player info error player id %d ", m_rankList[i][y].pid);
				continue;
			}
			...
			snprintf(rankingData.rank[i][y].szPlayerName, sizeof(rankingData.rank[i][y].szPlayerName), "%s", row[1]);
			str_to_number(rankingData.rank[i][y].playerRace, row[2]);

			//Country
			snprintf(szFetchQuery, sizeof(szFetchQuery), "SELECT language FROM account.account WHERE id = %d", accountId);

			 pMsg = CDBManager::instance().DirectQuery(szFetchQuery);
			if (pMsg->Get()->uiNumRows == 0)
			{
				sys_err("Ranking select player info error player id %d ", m_rankList[i][y].pid);
				continue;
			...
			str_to_number(rankingData.rank[i][y].country, row[0]);
		}
		if (maxRank != 10)
		{
			for (int t = m_rankList[i].size() - 1; t < RankingConf::RANKING_MAX_PLAYER_COUNT; t++)
			{
				rankingData.rank[i][t].bOrder = t;
			}
		}
	}

	memcpy(&m_rankingData, &rankingData, sizeof(m_rankingData));

	return true;
}

void CClientManager::SendRank(CPeer* pkPeer)
{
	TRankigDGPacket pack{};
	pack.header = HEADER_DG_GLOBAL_RANKING;
	pack.size = sizeof(m_rankingData) + sizeof(TRankigDGPacket);
	pack.subHeader = RANKING_TOP;

	TRankigDGPacket pack2{};
	pack2.header = HEADER_DG_GLOBAL_RANKING;
	pack2.subHeader = RANKING_PLAYER;

	if (pkPeer != nullptr)
	{
		pkPeer->EncodeHeader(HEADER_DG_GLOBAL_RANKING, 0, pack.size);
		pkPeer->Encode(&pack, sizeof(TRankigDGPacket));
		pkPeer->Encode(&m_rankingData, sizeof(TRankingData));

		for (BYTE i = 0; i < StatsTypes::RANKING_MAX; i++)
		{
			int rankListSize = m_rankList[i].size();
			pack2.size = sizeof(TRankigDGPacket) + sizeof(BYTE) + sizeof(int) + (rankListSize * sizeof(TRanking));

			pkPeer->EncodeHeader(HEADER_DG_GLOBAL_RANKING, 0, pack2.size);
			pkPeer->Encode(&pack2, sizeof(TRankigDGPacket));
			pkPeer->Encode(&i, sizeof(BYTE));	
			pkPeer->Encode(&rankListSize, sizeof(int));

			for (int y = 0; y < rankListSize; y++)
			{
				pkPeer->Encode(&m_rankList[i][y], sizeof(TRanking));
			}
		}
	}
	else
	{
		TEMP_BUFFER buff;
		buff.write(&pack, sizeof(TRankigDGPacket));
		buff.write(&m_rankingData, sizeof(TRankingData));

		ForwardPacket(HEADER_DG_GLOBAL_RANKING, buff.read_peek(), buff.size());
		...
		for (BYTE i = 0; i < StatsTypes::RANKING_MAX; i++)
		{
			...
			buff2.write(&pack2, sizeof(TRankigDGPacket));
			buff2.write(&i, sizeof(BYTE));
			buff2.write(&rankListSize, sizeof(int));

			for (int y = 0; y < rankListSize; y++)
			{
				buff2.write(&m_rankList[i][y], sizeof(TRanking));
			}
			ForwardPacket(HEADER_DG_GLOBAL_RANKING, buff2.read_peek(), buff2.size());
		}
	}
}
#endif