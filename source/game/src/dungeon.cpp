...
void CDungeon::DecMember(LPCHARACTER ch)
{
	...
	if (m_set_pkCharacter.empty())
	{
		...
		long timer = GetFlag("complete") > 0 ? PASSES_PER_SEC(5) : PASSES_PER_SEC(120);

		event_cancel(&deadEvent);
		deadEvent = event_create(dungeon_dead_event, info, timer);
	}
}
...
void CDungeon::UniqueSetMaxHP(const std::string& key, HP_LL iMaxHP)
{
...
void CDungeon::UniqueSetHP(const std::string& key, HP_LL iHP)
{
	TUniqueMobMap::iterator it = m_map_UniqueMob.find(key);
	if (it == m_map_UniqueMob.end())
	{
...
LPCHARACTER CDungeon::GetCharacter()
{
	if (m_set_pkCharacter.empty())
		return nullptr;
