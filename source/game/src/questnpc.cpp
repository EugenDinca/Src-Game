...
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
		CQuestManager& mgr = CQuestManager::instance();
		BYTE bLocale = mgr.GetCurrentCharacterPtr() ? mgr.GetCurrentCharacterPtr()->GetDesc() ? mgr.GetCurrentCharacterPtr()->GetDesc()->GetLanguage() : LOCALE_YMIR : LOCALE_YMIR;
#endif
...
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
			os << ", '" << LC_LOCALE_TEXT("TRANSLATE_CLOSE_QUESTNPC", bLocale) << "'";
#else
			os << ", '" << LC_TEXT("TRANSLATE_CLOSE_QUESTNPC") << "'";
#endif
			os << ")";
