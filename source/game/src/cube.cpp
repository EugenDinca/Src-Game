...
void Cube_init()
{
	...
	snprintf(file_name, sizeof(file_name), "/lightwork/main/srv1/share/locale/europe/common/cube.txt");
...
#ifdef ENABLE_EXTENDED_YANG_LIMIT
	int64_t cube_gold = cube_proto->gold;
#else
...
#endif
{
	...
	if (inven_type != INVENTORY && inven_type != UPGRADE_INVENTORY && inven_type != BOOK_INVENTORY && inven_type != STONE_INVENTORY && inven_type != CHEST_INVENTORY)
		return;
#else
...
void Cube_request_result_list(LPCHARACTER ch)
{
	...
	if (resultText.length() == 0)
	{
		...
		size_t resultTextSize = resultText.size() < 20 ? 20 - resultText.size() : resultText.size() - 20;//@Lightwork#9
		if (resultTextSize >= CHAT_MAX_LEN)//@Lightwork#9
		{
...
void Cube_request_material_info(LPCHARACTER ch, int requestStartIndex, int requestCount)
{
	...
	if (!bCatchInfo || materialInfoText.size() == 0) // @fixme127
		// if (false == bCatchInfo)
	{
