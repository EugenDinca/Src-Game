...
#else
	strlcpy(m_data.grade_array[0].grade_name, LC_TEXT("GUILD_TALK_28"), sizeof(m_data.grade_array[0].grade_name));
#endif
...
#else
		strlcpy(m_data.grade_array[i].grade_name, LC_TEXT("GUILD_TALK_21"), sizeof(m_data.grade_array[i].grade_name));
#endif
...
{
	if (str.length() > GUILD_COMMENT_MAX_LEN || str.length() == 0) // lightwork guild spam comment fixed.
		return;

	if (m_guildPostCommentPulse > thecore_pulse())// lightwork guild spam comment fixed.
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You can post new comment in: %02d mins and %02d secs!"), minutes, seconds);
		return;
	}
...
void CGuild::DeleteComment(LPCHARACTER ch, DWORD comment_id)
{
	...
	if (pmsg->Get()->uiAffectedRows == 0 || pmsg->Get()->uiAffectedRows == (uint32_t)-1)
		ch->NewChatPacket(GUILD_TALK_27);
	else
...
void CGuild::UseSkill(DWORD dwVnum, LPCHARACTER ch, DWORD pid)
{
	...
	{
		ch->NewChatPacket(GUILD_TALK_26);
		return;
	}
	...
	case GUILD_SKILL_TELEPORT:
		...
		else
		{
			if (m_memberP2POnline.find(pid) != m_memberP2POnline.end())
			{
				...
				{
					ch->NewChatPacket(GUILD_TALK_25, "%d|%d", pcci->bChannel, g_bChannel);
				}
				else
				{
				...
				ch->NewChatPacket(GUILD_TALK_24);
		}
		break;
...
void CGuild::RequestWithdrawMoney(LPCHARACTER ch, int iGold)
{
	...
	{
		ch->NewChatPacket(GUILD_TALK_23);
		return;
	}
...
void CGuild::Invite(LPCHARACTER pchInviter, LPCHARACTER pchInvitee)
{
	...
	case GERR_WITHDRAWPENALTY:
		pchInviter->ChatPacket(CHAT_TYPE_INFO,
			LC_TEXT("GUILD_TALK_14 %d"),
			quest::CQuestManager::instance().GetEventFlag("guild_withdraw_delay"));
		return;
	case GERR_COMMISSIONPENALTY:
		pchInviter->ChatPacket(CHAT_TYPE_INFO,
			LC_TEXT("GUILD_TALK_17 %d"),
			quest::CQuestManager::instance().GetEventFlag("guild_disband_delay"));
		return;
	case GERR_ALREADYJOIN:	pchInviter->NewChatPacket(GUILD_TALK_16); return;
	case GERR_GUILDISFULL:	pchInviter->NewChatPacket(GUILD_TALK_15); return;
	case GERR_GUILD_IS_IN_WAR: pchInviter->NewChatPacket(GUILD_TALK_18); return;
	case GERR_INVITE_LIMIT: pchInviter->NewChatPacket(GUILD_TALK_19); return;
...
void CGuild::InviteAccept(LPCHARACTER pchInvitee)
{
	...
	case GERR_COMMISSIONPENALTY:
		pchInvitee->ChatPacket(CHAT_TYPE_INFO,
			...
			quest::CQuestManager::instance().GetEventFlag("guild_disband_delay"));
		return;
	case GERR_ALREADYJOIN:	pchInvitee->NewChatPacket(GUILD_TALK_16); return;
	case GERR_GUILDISFULL:	pchInvitee->NewChatPacket(GUILD_TALK_15); return;
	case GERR_GUILD_IS_IN_WAR: pchInvitee->NewChatPacket(GUILD_TALK_18); return;
	case GERR_INVITE_LIMIT: pchInvitee->NewChatPacket(GUILD_TALK_19); return;
