...
namespace quest
{
	...
	ALUA(dungeon_setInfo)
	{
		...
		if (!ch)
		{
			sys_err("character find error");
			return 0;
		}

		if (!pDungeon)
		{
			sys_err("dungeon find error");
			return 0;
		}
		TDungeonPlayerInfo info;
		info.dungeonChannel = g_bChannel;
		info.dungeonMapIdx = pDungeon->GetMapIndex();

		ch->SetPlayerDungeonInfo(info);
		
		return 0;
	}

	ALUA(dungeon_turnBack)
	{
		long x = (long)lua_tonumber(L, 1);
		long y = (long)lua_tonumber(L, 2);
		...
		TDungeonPlayerInfo dungeonInfo = ch->GetPlayerDungeonInfo();

		TPGGDungeonTurnBack p;
		p.header = HEADER_GG_DUNGEON_TURN_BACK;
		p.subHeader = SUBHEADER_GG_DUNGEON_REQ;
		p.pid = ch->GetPlayerID();
		p.requestChannel = g_bChannel;
		p.dungeonChannel = dungeonInfo.dungeonChannel;
		p.mapIdx = dungeonInfo.dungeonMapIdx;
		p.x = x;
		p.y = y;
		
		P2P_MANAGER::instance().Send(&p, sizeof(TPGGDungeonTurnBack));
	...
	ALUA(dungeon_zodiac_chat)
	{
		...
		LPCHARACTER ch = pDungeon->GetCharacter();
		if (!ch)
			return 0;
...
#endif
#ifdef ENABLE_DUNGEON_P2P
			{ "setInfo",		dungeon_setInfo	},
			{ "turnBack",		dungeon_turnBack },
#endif
#ifdef ENABLE_NEW_CHAT
			{ "zodiac_chat", dungeon_zodiac_chat },
			{ "chat", dungeon_chat },
#endif
			{ "purge_vid",		dungeon_purge_vid },
			{ "gohome",		dungeon_gohome },
			{ NULL,				NULL			}
		};
