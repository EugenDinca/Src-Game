...
void DESC_MANAGER::DestroyDesc(LPDESC d, bool bEraseFromSet)
{
	...
	if(d->GetCharacter())
		if(d->GetCharacter()->IsAffectFlag(AFF_AFTERDEATH_SHIELD))
			d->GetCharacter()->RemoveShieldAffect();
#endif
...
#include <boost/algorithm/string.hpp>//@Lightwork#34
struct name_with_desc_func
{
	...
	{
		return d->GetCharacter() && boost::algorithm::to_lower_copy(std::string(d->GetCharacter()->GetName())) == boost::algorithm::to_lower_copy(std::string(m_name));//@Lightwork#34
	}
};
