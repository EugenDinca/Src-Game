...
void CWarMap::IncMember(LPCHARACTER ch)
{
	...
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You can join the guild war as a spectator."));
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("If you are a spectator <End View> button to exit will appear."));
	}
	...
	SendScorePacket(0, d);
	SendScorePacket(1, d);
	UpdateUserCount(); // lightwork update warmap info fixed UpdateUserCount is moved to here
}
...
void CWarMap::CheckWarEnd()
{
	...
	if (m_TeamData[0].iMemberCount == 0 || m_TeamData[1].iMemberCount == 0)
	{
		...
		Notice(LC_TEXT("No other guild members participated in the Guild War."));
		Notice(LC_TEXT("If no one logs in within 1 minute, the guild battle will end automatically."));
...
void CWarMap::Timeout()
{
	...
	{
		Notice(LC_TEXT("The guild battle ended early and it was decided as a draw. (less than 5 minutes)"));
		dwWinner = 0;
		dwLoser = 0;
	}
	else
