...
namespace quest
{
	...
	int _locale_quest(lua_State* L)
	{
		...
		if (lpChr != nullptr)
		{
			...
			lua_pushstring(L, LC_LOCALE_QUEST_TEXT(lua_tonumber(L, 1), bLocale));
		}
		else
			lua_pushstring(L, "");
		return 1;
	...
	struct FWarpAllToVillage
	{
		...
		void operator()(LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (ch->IsPC() && !ch->IsGM())//@Lightwork#40
				{
	...
	ALUA(_warp_all_to_village)
	{
		...
		SendNoticeMap(LC_TEXT("Everyone will be teleported to the town."), iMapIndex, false);
	...
	ALUA(_add_ox_quiz)
	{
		...
		COXEventManager::instance().AddQuiz(level, quiz, answer);
