...
MAX_COUNT	g_bItemCountLimit = SET_MAX_COUNT;
#else
...
long long g_maxAlignment = 200000;
#endif

#ifdef ENABLE_AVERAGE_PRICE
int g_AveragePriceUpdateTime = 10 * 60;
#endif

#ifdef ENABLE_CLIENT_VERSION_AND_ONLY_GM
bool g_serverState = true;
int g_clientVersion[2] = { 0, };
#endif
...
{
	std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("SELECT version, player_mode FROM common.game_settings"));
	...
	if (!msg->uiSQLErrno)
	{
		if (msg->Get()->uiNumRows)
		{
			...
			{
				int newKey = 0;
				str_to_number(newKey, row[0]);
				str_to_number(g_serverState, row[1]);
				if (newKey != g_clientVersion[0])
				{
					g_clientVersion[1] = g_clientVersion[0];
					g_clientVersion[0] = newKey;
				}
			}
		}
	}
