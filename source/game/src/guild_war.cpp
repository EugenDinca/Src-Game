...
void CGuild::RequestDeclareWar(DWORD dwOppGID, BYTE type)
{
	...
	if (it == m_EnemyGuild.end())
	{
		if (!GuildWar_IsWarMap(type))
		{
			...
			map_allow_log();
			NotifyGuildMaster(LC_TEXT("GUILD_TALK_3"));
			return;
		}
	...
	{
		NotifyGuildMaster(LC_TEXT("GUILD_TALK_2"));
	}
	break;
	default:
...
void CGuild::EndWar(DWORD dwOppGID)
{
	...
	if (it != m_EnemyGuild.end())
	{
		...
		if (!UnderAnyWar())
		{
			...
			for (auto it_char = m_memberOnline.begin(); it_char != m_memberOnline.end(); ++it_char)
			{
...
void CGuild::SetLadderPoint(int point)
{
	if (m_data.ladder_point != point)
	{
		...
		char buf[256];
		snprintf(buf, sizeof(buf), LC_TEXT("GUILD_TALK_1 %d"), point);
#endif
		for (itertype(m_memberOnline) it = m_memberOnline.begin(); it != m_memberOnline.end(); ++it)
		{
			LPCHARACTER ch = (*it);
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
			ch->NewChatPacket(GUILD_TALK_1, "%d", point);
#else
