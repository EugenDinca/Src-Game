...
void ShutdownOnFatalError()
{
	if (!g_bShutdown)
	{
		sys_err("ShutdownOnFatalError!!!!!!!!!!");
		{
			...
			SendNotice(LC_TEXT("Critical server error. The server will reboot automatically."));
			SendNotice(LC_TEXT("You will be disconnected in 10 seconds."));
			SendNotice(LC_TEXT("You can connect again after 5 minutes."));
#else
			char buf[256];

			strlcpy(buf, LC_TEXT("Critical server error. The server will reboot automatically."), sizeof(buf));
			SendNotice(buf);
...
void heartbeat(LPHEART ht, int pulse)
{
	...
	if (!(pulse % (passes_per_sec * g_AveragePriceUpdateTime)))
	{
		offlineshop::CShopManager::Instance().AveragePriceUpdate();
	}
#endif
	...
	{
		CBossRegen::Instance().UpdateRegen();
	}
#endif
...
int main(int argc, char** argv)
{
...
#endif
#ifdef ENABLE_IN_GAME_LOG_SYSTEM
	InGameLog::InGameLogManager ingamelogManager;
#endif
#ifdef ENABLE_HWID
	CHwidManager hwidManager;
#endif
#ifdef ENABLE_ALIGNMENT_SYSTEM
	CAlignmentProto alignmentProto;
#endif
#ifdef ENABLE_DUNGEON_INFO
	CDungeonInfo dungeonInfo;
#endif
	if (!start(argc, argv)) {
		CleanUpForEarlyExit();
		return 0;
	...
	biologmanager.LoadInformationsData();
#endif
#ifdef ENABLE_BOSS_REGEN
	CBossRegen bossRegen;
#endif
...
#ifdef ENABLE_GLOBAL_RANKING
	sys_log(0, "<shutdown> Destroying statisticsRanking ...");
	statisticsRanking.Destroy();
#endif
#ifdef ENABLE_BIOLOG_SYSTEM
	biologmanager.Destroy();
#endif
	destroy();
#ifdef DEBUG_ALLOC
