...
#else
#ifdef IMPROVED_PACKET_ENCRYPTION_REMOVE
		if (!m_bEncrypted)
		{
			if (!packet_encode(m_lpOutputBuffer, c_pvData, iSize))
			{
...
void DESC::DisconnectOfSameLogin()
{
	if (GetCharacter())
	{
		...
		GetCharacter()->NewChatPacket(CMD_TALK_20);
		DelayedDisconnect(5);
	}
	else
	{
...
{
	if (m_accountTable.bLanguage)
		return m_accountTable.bLanguage;
...
void DESC::NewChatPacket(WORD chatID, const char* format, ...)
{
	if (!format)
		return;
...
void DESC::NewChatPacket(WORD chatID)
{
	TPacketNewChat p;
	p.header = HEADER_GC_NEW_CHAT;
