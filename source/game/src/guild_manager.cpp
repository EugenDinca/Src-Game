...
DWORD CGuildManager::CreateGuild(TGuildCreateParameter& gcp)
{
	...
	{
		gcp.master->NewChatPacket(GUILD_TALK_13);
		return 0;
	}
	...
	if (pmsg->Get()->uiNumRows > 0)
	{
		...
		if (!(row[0] && row[0][0] == '0'))
		{
			gcp.master->NewChatPacket(GUILD_TALK_12);
			return 0;
		}
	}
	else
	{
		gcp.master->NewChatPacket(GUILD_TALK_11);
		return 0;
	}
...
void CGuildManager::DeclareWar(DWORD guild_id1, DWORD guild_id2, BYTE bType)
{
		...
		g2->DeclareWar(guild_id1, bType, GUILD_WAR_RECV_DECLARE))
	{
		// @warme005
		{
			...
			SendNotice(LC_TEXT("GUILD_TALK_10 %s %s"), false, TouchGuild(guild_id1)->GetName(), TouchGuild(guild_id2)->GetName());
#else
			char buf[256];
			snprintf(buf, sizeof(buf), LC_TEXT("GUILD_TALK_10 %s %s"), TouchGuild(guild_id1)->GetName(), TouchGuild(guild_id2)->GetName());
			SendNotice(buf);
#endif
		}
	}
...
void CGuildManager::RefuseWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	if (g1 && g2)
	{
		if (g2->GetMasterCharacter())
			g2->GetMasterCharacter()->NewChatPacket(GUILD_TALK_9, "%s", g1->GetName());
	}
...
void CGuildManager::WaitStartWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	{
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
		SendNotice(LC_TEXT("GUILD_TALK_8 %s %s"), false, g1->GetName(), g2->GetName());
#else
		char buf[256];
		snprintf(buf, sizeof(buf), LC_TEXT("GUILD_TALK_8 %s %s"), g1->GetName(), g2->GetName());
		SendNotice(buf);
#endif
	}
}
...
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
	SendNotice(LC_TEXT("GUILD_TALK_7 %s %s"), false, g1->GetName(), g2->GetName());
#else
	char buf[256];
	snprintf(buf, sizeof(buf), LC_TEXT("GUILD_TALK_7 %s %s"), g1->GetName(), g2->GetName());
	SendNotice(buf);
#endif
...
void SendGuildWarOverNotice(CGuild* g1, CGuild* g2, bool bDraw)
{
	if (g1 && g2)
	{
		...
		if (bDraw)
		{
			SendNotice(LC_TEXT("GUILD_TALK_6 %s %s"), false, g1->GetName(), g2->GetName());
		}
		else
		{
			if (g1->GetWarScoreAgainstTo(g2->GetID()) > g2->GetWarScoreAgainstTo(g1->GetID()))
				SendNotice(LC_TEXT("GUILD_TALK_5 %s %s"), false, g1->GetName(), g2->GetName());
			else
				SendNotice(LC_TEXT("GUILD_TALK_5 %s %s"), false, g2->GetName(), g1->GetName());
		}
#else
		char buf[256];
		...
		if (bDraw)
		{
			snprintf(buf, sizeof(buf), LC_TEXT("GUILD_TALK_6 %s %s"), g1->GetName(), g2->GetName());
		}
		else
		{
			if (g1->GetWarScoreAgainstTo(g2->GetID()) > g2->GetWarScoreAgainstTo(g1->GetID()))
			{
				snprintf(buf, sizeof(buf), LC_TEXT("GUILD_TALK_5 %s %s"), g1->GetName(), g2->GetName());
			}
			else
			{
				snprintf(buf, sizeof(buf), LC_TEXT("GUILD_TALK_5 %s %s"), g2->GetName(), g1->GetName());
			}
		}
...
void CGuildManager::CancelWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	if (g1)
	{
		...
		if (master1)
			master1->NewChatPacket(GUILD_TALK_5);
	}
	...
	if (g2)
	{
		...
		if (master2)
			master2->NewChatPacket(GUILD_TALK_5);
	}

	if (g1 && g2)
	{
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
		SendNotice(LC_TEXT("%s GUILD_TALK_4 %s"), false, g1->GetName(), g2->GetName());
#else
		char buf[256 + 1];
		snprintf(buf, sizeof(buf), LC_TEXT("%s GUILD_TALK_4 %s"), g1->GetName(), g2->GetName());
		SendNotice(buf);
#endif
	}
}
