...
DAM_LL GetPoisonDamageRate(LPCHARACTER ch)
{
	DAM_LL iRate;
...
EVENTFUNC(poison_event)
{
	...
	DAM_LL dam = ch->GetMaxHP() * GetPoisonDamageRate(ch) / 1000;
...
DAM_LL GetBleedingDamageRate(LPCHARACTER ch)
{
	DAM_LL iRate;
...
EVENTFUNC(bleeding_event)
{
	...
	DAM_LL dam = ch->GetMaxHP() * GetBleedingDamageRate(ch) / 1000;
...
void CHARACTER::AttackedByFire(LPCHARACTER pkAttacker, DAM_LL amount, int count)
{
	if (m_pkFireEvent)
		return;
