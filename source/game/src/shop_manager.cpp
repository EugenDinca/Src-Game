...
bool CShopManager::StartShopping(LPCHARACTER pkChr, LPCHARACTER pkChrShopKeeper, int iShopVnum)
{
	...
	if (pkChr->IsOpenSafebox() || pkChr->GetExchange() || pkChr->GetMyShop() || pkChr->IsCubeOpen() || pkChr->IsAcceOpened() || pkChr->IsOpenOfflineShop()
#ifdef ENABLE_AURA_SYSTEM
		|| pkChr->isAuraOpened(true) || pkChr->isAuraOpened(false)
#endif
	...
	{
		pkChr->NewChatPacket(CLOSE_WINDOWS_ERROR);
		return false;
	}
...
LPSHOP CShopManager::CreatePCShop(LPCHARACTER ch, TShopItemTable* pTable, MAX_COUNT bItemCount)
#else
...
void CShopManager::DestroyPCShop(LPCHARACTER ch)
{
	...
	//PREVENT_ITEM_COPY;
	ch->SetActivateTime(MYSHOP_CHECK_TIME);
...
#ifndef ENABLE_DISTANCE_NPC
	if (DISTANCE_APPROX(ch->GetX() - ch->GetShopOwner()->GetX(), ch->GetY() - ch->GetShopOwner()->GetY()) > 2000)
	{
		ch->NewChatPacket(STRING_D92);
		return;
	}
#endif
...
void CShopManager::Sell(LPCHARACTER ch, CELL_UINT bCell,
#ifdef ENABLE_EXTENDED_COUNT_ITEMS
	MAX_COUNT bCount
#else
	BYTE bCount
...
)
{
	...
	DWORD dwTax = 0;//@fixme991 tax /5 fix
	int iVal = 0;
	...
	if (GOLD_MAX <= nTotalMoney)
	{
		...
		ch->NewChatPacket(STRING_D94);
		return;
	}

	if (iVal > 0)
		ch->NewChatPacket(STRING_D114, "%d", iVal);
	...
	log.dwCount = bCount;
	log.pid = ch->GetPlayerID();
	snprintf(log.logType, sizeof(log.logType), "SHOP_SELL");
