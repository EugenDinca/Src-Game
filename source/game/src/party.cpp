	...
	while (it != m_memberMap.end())
	{
		...
		if (rMember.pCharacter)
		{
			if (rMember.pCharacter->GetDesc())
			{
				...
				rMember.pCharacter->NewChatPacket(STRING_D102);
			}
			else
			{
	...
	if (it == m_memberMap.end())
	{
		...
		if (m_bPCParty)
		{
			...
			if (ch)
			{
				...
				ComputeRolePoint(ch, Member.bRole, false);
#endif
			}
			else
			{
...
void CParty::SummonToLeader(DWORD pid)
{
	...
	{
		l->NewChatPacket(STRING_D104);
		return;
	}

	if (!ch->CanSummon(m_iLeadership))
	{
		l->NewChatPacket(STRING_D105);
		return;
	}
