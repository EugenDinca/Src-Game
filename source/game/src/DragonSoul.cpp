...
{
	return bType * 10000 + grade * 1000 + step * 100 + refine * 10; // lightwork correction ds 10 > 11
}
...

	{
		...
		else
		{
			...
			if (dwByProduct)
			{
				...
				if (pByProduct)
					ch->NewChatPacket(DS_TALK_6, "%s", pByProduct->GetName());
				else
					ch->NewChatPacket(DS_TALK_3);
			}
			else
...
#ifndef ENABLE_RANGE_DRAGON_SOUL_UPGRADE
...
bool DSManager::DoRefineStep(LPCHARACTER ch, TItemPos(&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	{
		ch->NewChatPacket(IT_WAS_FAILED);
		SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL, TItemPos(pResultItem->GetWindow(), pResultItem->GetCell()));
		return false;
	}
}
