...
namespace quest
{
	...
	ALUA(pc_give_item_from_special_item_group)
	{
		...
		for (int i = 0; i < count; i++)
		{
			if (!item_gets[i])
			{
				if (dwVnums[i] == 1)
				{
					ch->NewChatPacket(STRING_D98, "%d", dwCounts[i]);
				}
				else if (dwVnums[i] == 2)
				{
					ch->NewChatPacket(STRING_D97);
					ch->NewChatPacket(STRING_D96, "%d", dwCounts[i]);
				}
			}
		}
		return 0;
	...
	ALUA(pc_get_wear)
	{
		...
		CELL_UINT bCell = (CELL_UINT)lua_tonumber(L, 1);
	...
	ALUA(pc_set_skillgroup)
	{
		...
		else
		{
			CQuestManager& q = CQuestManager::Instance();
			LPCHARACTER ch = q.GetCurrentCharacterPtr();
			ch->RemoveGoodAffect();//@Lightwork#6
	...
	ALUA(pc_select_vid)
	{
		...
		if (new_ch)
		{
			...
			lua_pushnumber(L, ch ? (DWORD)ch->GetVID() : 0);//@Lightwork#12
		}
		else
		{
			lua_pushnumber(L, 0);
...
ALUA(pc_open_guild_ranking)
{
    ...
    std::unique_ptr<SQLMsg> pMsg(
        DBManager::instance().DirectQuery(
            "SELECT guild.id, guild.name AS guild_name, player.name AS guild_owner, "
            "guild.level, guild.win, guild.draw, guild.loss, guild.ladder_point "
            "FROM player.guild "
            "INNER JOIN player.player ON player.id = guild.master "
            "ORDER BY guild.level DESC, guild.win DESC, guild.draw DESC, "
            "guild.ladder_point DESC, guild.loss DESC "
            "LIMIT %d",
            GUILD_RANK::GUILD_RANKING_MAX_NUM
        )
    );
    ...
    while ((row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
    {
        ...
        const TPacketGCGuildRankingSend t(
            ID, GuildName, OwnerName, Level, Point, Win, Draw, Loss
        );
...
#define PC_MI0L_ARG3	7*2		// (type, value)*7
	...
	int pc_get_language(lua_State* L)
	{
		...
		if (ch)
		{
			lua_pushstring(L, LC_LOCALE(ch->GetLanguage()));
			return 1;
		}
	...
	ALUA(pc_complete_dungeon)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!ch)
			return 0;

		ch->PointChange(POINT_COMPLATE_DUNGEON, 1);
	...
	ALUA(pc_zodiac_chat)
	{
		...
		if (lua_isstring(L, 2) && lua_isstring(L, 3))
		{
			if (lua_isstring(L, 4))
			{
				if (lua_isstring(L, 5))
				{
					ch->QuestChatPacket(lua_tonumber(L, 1), CHAT_TYPE_ZODIAC_NOTICE, lua_tostring(L, 2), lua_tostring(L, 3), lua_tostring(L, 4), lua_tostring(L, 5));
				}
				else
				{
					ch->QuestChatPacket(lua_tonumber(L, 1), CHAT_TYPE_ZODIAC_NOTICE, lua_tostring(L, 2), lua_tostring(L, 3), lua_tostring(L, 4));
				}
			}
			else
			{
				ch->QuestChatPacket(lua_tonumber(L, 1), CHAT_TYPE_ZODIAC_NOTICE, lua_tostring(L, 2), lua_tostring(L, 3));

			}
		}
		else
		{
			ch->QuestChatPacket(lua_tonumber(L, 1), CHAT_TYPE_ZODIAC_NOTICE);
		}
		return 1;
	}
	...
	ALUA(pc_chat)
	{
		...
		if (lua_isstring(L, 2) && lua_isstring(L, 3))
		{
			if (lua_isstring(L, 4))
			{
				if (lua_isstring(L, 5))
				{
					ch->QuestChatPacket(lua_tonumber(L, 1), CHAT_TYPE_NOTICE, lua_tostring(L, 2), lua_tostring(L, 3), lua_tostring(L, 4), lua_tostring(L, 5));
				}
				else
				{
					ch->QuestChatPacket(lua_tonumber(L, 1), CHAT_TYPE_NOTICE, lua_tostring(L, 2), lua_tostring(L, 3), lua_tostring(L, 4));
				}
			}
			else
			{
				ch->QuestChatPacket(lua_tonumber(L, 1), CHAT_TYPE_NOTICE, lua_tostring(L, 2), lua_tostring(L, 3));

			}
		}
		else
		{
			ch->QuestChatPacket(lua_tonumber(L, 1), CHAT_TYPE_NOTICE);
		}
		return 1;
	}
#endif
	...
	ALUA(pc_goto)
	{
		...
		ch->Show(ch->GetMapIndex(), lua_tonumber(L,1)*100, lua_tonumber(L, 2)*100, ch->GetZ());
	...
	void RegisterPCFunctionTable()
	{
		luaL_reg pc_functions[] =
		{
...
#endif
#ifdef ENABLE_6TH_7TH_ATTR
			{"get_67attr_item",		pc_get_67attr_item},
			{"give_67attr_item",	pc_give_67attr_item},
#endif
#ifdef ENABLE_NEW_CHAT
			{"zodiac_chat", pc_zodiac_chat },
			{"chat", pc_chat },
#endif
#ifdef ENABLE_PLAYER_STATS_SYSTEM
			...
			{ "goto", pc_goto },
			{ "rewarp", pc_rewarp },
			{ "window_close_all", pc_window_close_all },
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
