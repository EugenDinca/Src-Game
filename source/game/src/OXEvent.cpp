...
bool COXEventManager::ShowQuizList(LPCHARACTER pkChar)
{
	...
	for (size_t i = 0; i < m_vec_quiz.size(); ++i)
	{
		...
		{
			pkChar->ChatPacket(CHAT_TYPE_INFO, "%d %s %s", m_vec_quiz[i][j].level, m_vec_quiz[i][j].Quiz, m_vec_quiz[i][j].answer ? LC_TEXT("OX_TALK_1") : LC_TEXT("OX_TALK_9"));
		}
	}

	pkChar->NewChatPacket(OX_TALK_10, "%d", c);
	return true;
}
...
EVENTFUNC(oxevent_timer)
{
	...
	{
	case 0:
		SendNoticeMap(LC_TEXT("OX_TALK_8"), OXEVENT_MAP_INDEX, true);
		flag++;
	...
	case 1:
		...
		if (info->answer == true)
		{
			...
			SendNoticeMap(LC_TEXT("OX_TALK_6"), OXEVENT_MAP_INDEX, true);
		}
		else
		{
			COXEventManager::instance().CheckAnswer(false);
			SendNoticeMap(LC_TEXT("OX_TALK_5"), OXEVENT_MAP_INDEX, true);
		}

		SendNoticeMap(LC_TEXT("OX_TALK_4"), OXEVENT_MAP_INDEX, true);
...
bool COXEventManager::Quiz(unsigned char level, int timelimit)
{
	...
	SendNoticeMap(LC_TEXT("OX_TALK_2"), OXEVENT_MAP_INDEX, true);
	SendNoticeMap(m_vec_quiz[level][idx].Quiz, OXEVENT_MAP_INDEX, true);
	SendNoticeMap(LC_TEXT("OX_TALK_3"), OXEVENT_MAP_INDEX, true);
...
bool COXEventManager::CheckAnswer(bool answer)
{
	...
	for (; iter != m_map_attender.end();)
	{
		...
		if (pkChar != NULL)
		{
			...
			{
				pkChar->NewChatPacket(OX_TALK_1);
				char chatbuf[256];
...
bool COXEventManager::CheckIpAddress(LPCHARACTER ch)
{
	for (itertype(m_map_attender) it = m_map_attender.begin(); it != m_map_attender.end(); ++it)
	{
		...
		if (!strcmp(ch->GetDesc()->GetHostName(), tch->GetDesc()->GetHostName()) && ch->GetMapIndex() == tch->GetMapIndex())
		{
			...
			tch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Dinasty2] Cannot connect more character with same IP to OX Event."));
			ch->GoHome();
			return false;
		}
	}
...
bool COXEventManager::GiveItemToAttender(DWORD dwItemVnum, MAX_COUNT count)
#else
