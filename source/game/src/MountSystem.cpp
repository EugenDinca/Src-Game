...
void CMountActor::SetName()
{
	...
	if (m_pkOwner->FindAffect(AFFECT_NAMING_SCROLL_MOUNT))
	{
		snprintf(buf, sizeof(buf), "%s", m_pkOwner->GetMobNameScroll(MOUNT_NAME_NUM));
	}
	else
	{
...
#ifdef ENABLE_FISHING_RENEWAL
	if (m_pkOwner->m_pkFishingEvent)
	{
		m_pkOwner->m_bFishGameFishCounter = 0;
		m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "OpenFish 0");
		event_cancel(&m_pkOwner->m_pkFishingEvent);
	}
#endif

#ifdef ENABLE_MINING_RENEWAL
	if (m_pkOwner->m_pkMiningEvent)
	{
		m_pkOwner->EndMiningEvent();
		event_cancel(&m_pkOwner->m_pkMiningEvent);
	}
#endif

	m_pkOwner->MountVnum(m_dwVnum);

	if (m_pkOwner->FindAffect(SKILL_MUYEONG))
		m_pkOwner->RemoveAffect(SKILL_MUYEONG);
...
void CMountActor::Unsummon()
{
	if (true == this->IsSummoned())
	{
		...
		if (m_pkChar != NULL || CHARACTER_MANAGER::Instance().Find(this->GetVID()) != NULL)
...
{
	if (m_pkOwner->GetMapIndex() == 113) // // In ox you can't summon your pet.
		return 0;
...
bool CMountSystem::Update(DWORD deltaTime)
{
	...
	for (auto iter = m_mountActorMap.begin(); iter != m_mountActorMap.end(); ++iter)
	{
		...
		if (0 != mountActor && mountActor->IsSummoned())
		{
			...
			if (!pMount || CHARACTER_MANAGER::instance().Find(pMount->GetVID()) == nullptr)
			{
