...
void CPrivManager::GiveGuildPriv(DWORD guild_id, BYTE type, int value, BYTE bLog, time_t end_time_sec)
{
	...
	if (g)
	{
		if (value)
		{
			...
			SendGuildPrivNotice(LC_TEXT("Item drop rate for guild %s is increased %d%%"), g->GetName(), GetPrivName(type), value);
#else
			char buf[100];
			snprintf(buf, sizeof(buf), LC_TEXT("Item drop rate for guild %s is increased %d%%"), g->GetName(), GetPrivName(type), value);
			SendNotice(buf);
#endif
		}
		else
		{
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
			SendGuildPrivNotice(LC_TEXT("Item drop rate for guild %s is increased"), g->GetName(), GetPrivName(type));
#else
			char buf[100];
			snprintf(buf, sizeof(buf), LC_TEXT("Item drop rate for guild %s is increased %d%%"), g->GetName(), GetPrivName(type));
			SendNotice(buf);
#endif
		}
	}
...
void CPrivManager::GiveEmpirePriv(BYTE empire, BYTE type, int value, BYTE bLog, time_t end_time_sec)
{
	...
	{
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
		SendPrivNotice(LC_TEXT("%s rate for guild %s is increased %d%%"), GetEmpireName(empire), GetPrivName(type), value);
#else
		char buf[100];
		snprintf(buf, sizeof(buf), LC_TEXT("%s rate for guild %s is increased %d%%"), GetEmpireName(empire), GetPrivName(type), value);
	...
	{
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
		SendPrivNotice(LC_TEXT("%s rate for %s is returned to the default value %d%%"), GetEmpireName(empire), GetPrivName(type));
#else
		char buf[100];
		snprintf(buf, sizeof(buf), LC_TEXT("%s rate for %s is returned to the default value %d%%"), GetEmpireName(empire), GetPrivName(type));
