...
static DWORD GiveBasicArmorParts(BYTE race)
{
	switch (race)
	{
	case 0:
...
void CInputLogin::CharacterCreate(LPDESC d, const char* data)
{
	...
	if (strlen(pinfo->name) > 12)// @Lightwork#17
	{
...
#if !defined(__BINARY_ATLAS_MARK_INFO__) && !defined(ENABLE_MULTI_LANGUAGE_SYSTEM)
...
/* Lightwork@588 */
	case HEADER_CG_MARK_LOGIN:
		break;
		/* Lightwork@588 login phase does not handle this packet! header 100 fix */
